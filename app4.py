...
def main():
    ...
    if not df.empty:
        ...
        visualize_candles = min(visualize_candles, len(df))
        ...
        max_candle = len(df) - 1
        candle_min_required = backcandles + channel_window + 5
        candle_max = max_candle

        if candle_max >= candle_min_required:
            # Calculate a sensible default value
            default_candle = min(75, candle_max)
            candle = st.number_input(
                "Select Candle Index for Channel Detection",
                min_value=candle_min_required,
                max_value=candle_max,
            ...
            st.warning(
                f"Not enough data points for the selected parameters. "
                f"Minimum required candles: {candle_min_required}. "
                f"Available candles: {len(df)}. "
                f"Please select a larger date range or adjust the parameters."
            )
        ...
        if total_breakouts > 0:
            ...
            breakouts_df = df[df['BreakOut'].isin([1, 2])].copy()
            breakouts_df['breakpointpos'] = breakouts_df.apply(calculate_breakpoint_position, axis=1)
        ...
        if total_breakouts > 0:
            ...
            if breakout_type == "All":
                filtered_breakouts = df[df['BreakOut'].isin([1, 2])].copy()
            elif breakout_type == "Breakout Down":
                filtered_breakouts = df[df['BreakOut'] == 1].copy()
            else:
                filtered_breakouts = df[df['BreakOut'] == 2].copy()

            # Calculate 'breakpointpos' for the filtered breakouts
            filtered_breakouts['breakpointpos'] = filtered_breakouts.apply(calculate_breakpoint_position, axis=1)

            # Remove any breakouts without a valid 'breakpointpos'
            ...
            if not breakouts_to_plot.empty:
                ...
                # To visualize all breakouts, plot the entire dataset with markers
                df_vis = df.copy()
                ...
                st.info("No breakouts to visualize after filtering.")
        else:
